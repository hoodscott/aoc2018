#https://github.com/narimiran/AdventOfCode2018/blob/master/nim/day22.nim
import heapqueue
  ...
  while queue.len > 0:
    ...
    for neighbour in current.pos.neighbours:
      if neighbour.isValidCoord and current.tool != Tool(regMap[neighbour]):
        ...
        if minutes[current.tool][neighbour] == 0 or time < minutes[
            current.tool][neighbour]:
